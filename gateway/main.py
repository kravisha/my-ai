"""The AI Communication Gateway (addenda 16 and 17), increment G1.
...
What exists in G1: the boundary, Super User authentication, and a streaming text
conversation with the analysis model, persisted so a reconnect resumes it. What
does not: voice, Git, the Scoreboard, and any call into the backend at all. This
service does not yet touch Jarvis.
...
from gateway import auth, conversation, store
...
@app.websocket("/ws")
async def conversation_socket(websocket: WebSocket, conn=Depends(gateway_db)):
    ...
    **The token arrives as the first message, not in the URL.** Browsers cannot
    ...
    while True:
        ...
        fragments: list[str] = []
        try:
            async for fragment in iterate_in_thread(
                lambda: conversation.reply_fragments(history, provider)
            ):
                fragments.append(fragment)
                await websocket.send_json({"type": "delta", "text": fragment})
        except WebSocketDisconnect:
            ...
            if fragments:
                conversation.record_assistant_message(conn, conversation_id, "".join(fragments))
            return
        ...
        message_id = conversation.record_assistant_message(
            conn, conversation_id, "".join(fragments)
        )
