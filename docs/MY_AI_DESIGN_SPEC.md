...
**Built (test infrastructure)**: a `pytest` suite (`tests/`, 65 tests) covering every module in `app/`, including a mocked-model regression suite for the chat loop (`tests/test_chat_turn.py`) that locks in the two real bugs found during Milestone 2 verification — no client-side caching of tool results across turns, and the two denial kinds (permission-revoked vs. forwarding-disposition-`never`) reaching the model with distinct `error` text. Fully hermetic: no real API key or network access required (`call_reasoning_model` is mocked wherever the chat loop is exercised), and all persisted state is redirected to `tmp_path` fixtures. Writing it surfaced one real bug, fixed as part of this work: `PrivacyPreferenceStore.list_all()` returned a shallow copy, so a caller mutating a returned entry could silently corrupt the store's actual state.
