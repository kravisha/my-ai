...
def test_there_is_no_way_to_write_the_text_except_amending():
    """*"They are read only principles to act by."* The only mutating entry
    points are `install`, which refuses to overwrite, and `amend`, which needs
    the owner's key."""
    exported = {name for name in dir(constitution) if not name.startswith("_")}
    assert "write" not in exported
    assert {"read", "install", "amend", "verify"} <= exported
...
def test_jarvis_cannot_amend_without_the_owners_key(client, sealed):
    with pytest.raises(constitution.Unauthorised) as raised:
        constitution.amend(client, "Jarvis may do as he pleases.", key=sealed,
                           why="convenience")
...
def test_with_the_owners_key_an_amendment_lands(operator, client, sealed):
    _granted(operator)
    entry = constitution.amend(client, "Jarvis serves Krish, and rests on Sundays.",
                               key=sealed, why="Krish asked for Sundays")
...
def test_an_amendment_must_say_why(operator, client, sealed):
    ...
    with pytest.raises(ValueError, match="must say why"):
        constitution.amend(client, "new text", key=sealed, why="  ")
    assert constitution.amendments(key=sealed) == []
...
def test_a_lapsed_key_cannot_amend(operator, client, sealed):
    ...
    with pytest.raises(constitution.Unauthorised, match="lapsed"):
        constitution.amend(client, "new text", key=sealed, why="trying anyway")
...
def test_the_circular_key_does_not_amend_the_constitution(operator, client, sealed):
    ...
    with pytest.raises(constitution.Unauthorised):
        constitution.amend(client, "new text", key=sealed, why="wrong key")


def test_an_amendment_is_recorded_in_the_life_ledger(operator, client, sealed):
    _granted(operator)
    constitution.amend(client, "amended text", key=sealed, why="Krish said so")
...
def test_a_clean_history_verifies(operator, client, sealed):
    _granted(operator)
    constitution.amend(client, "second", key=sealed, why="first change")
    _granted(operator)
    constitution.amend(client, "third", key=sealed, why="second change")
...
def test_rewriting_the_sealed_text_directly_is_detected(operator, client, sealed):
    ...
    can produce a perfectly valid sealed file - and it will not match the chain."""
    _granted(operator)
    constitution.amend(client, "lawful text", key=sealed, why="a real amendment")
    ...
    assert any("does not match the newest amendment" in problem
...
def test_an_amendment_naming_a_grant_that_was_never_issued_is_detected(
    ...
    _granted(operator)
    constitution.amend(client, "lawful", key=sealed, why="real")
...
def test_altering_an_old_amendment_is_detected(operator, client, sealed):
    _granted(operator)
    constitution.amend(client, "second", key=sealed, why="the real reason")
    _granted(operator)
    constitution.amend(client, "third", key=sealed, why="another")
...
def test_removing_an_amendment_from_the_middle_is_detected(operator, client, sealed):
    ...
    for index in range(3):
        _granted(operator)
        constitution.amend(client, f"version {index}", key=sealed,
                           why=f"change {index}")
