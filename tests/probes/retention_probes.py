...
PROBES: list[harness.Probe] = [
    ...
    (
        RETENTION,
        "an untested policy reports a clean record instead of saying it is untested",
        "    if not collections:",
        "    if False:",
        ("test_an_untested_policy_says_so_rather_than_reporting_a_clean_record",),
    ),
    (
        RETENTION,
        "the policy is never judged to be too aggressive",
        "    if rate > REGRET_RATE_TOO_HIGH:",
        "    if False:",
        ("test_too_many_regrets_fails_the_policy_and_names_the_remedy",),
    ),
    (
        RETENTION,
        "any regret at all fails the policy, so collecting becomes impossible",
        "REGRET_RATE_TOO_HIGH = 0.1",
        "REGRET_RATE_TOO_HIGH = 0.0",
        ("test_a_few_regrets_are_the_price_of_collecting_anything",
         "test_the_policy_numbers_are_what_they_are"),
    ),
    (
        RETENTION,
        "the failure verdict does not say what to change",
        '                   f"are too aggressive: raise CYCLES_BEFORE_COLD or "',
        '                   f"are too aggressive. "',
        ("test_too_many_regrets_fails_the_policy_and_names_the_remedy",),
    ),
    (
        RETENTION,
        "regrets are counted from one kind instead of all of them",
        "    regrets = sum(item.regretted for item in histories)",
        "    regrets = next((item.regretted for item in histories), 0)",
        ("test_regrets_are_counted_across_every_kind",),
    ),
    (
        RETENTION,
        "the policy retunes its own threshold from its own regrets",
        "    rate = regrets / collections",
        "    rate = regrets / collections\n"
        "    REGRET_RATE_TOO_HIGH = rate",
        ("test_the_policy_reports_its_own_failure_and_never_retunes_itself",),
    ),
    (
        RETENTION,
        "a tombstone is kept for ever",
        "TOMBSTONE_HORIZON_DAYS = 3 * DEFAULT_INTERVAL_DAYS",
        "TOMBSTONE_HORIZON_DAYS = 3 * 365 * 100",
        ("test_the_policy_numbers_are_what_they_are",),
    ),
    (
        RETENTION,
