...
def test_scan_for_anomaly_finds_the_forced_peak():
    provider = SyntheticMarketDataProvider(seed=42, force_anomaly=True)
...
def test_scan_for_anomaly_low_ratio_without_forced_anomaly():
    provider = SyntheticMarketDataProvider(seed=42, force_anomaly=False)
    surface = provider.get_option_surface("SYN1")
...
def test_real_explorer_agent_detects_and_files_report(tmp_path):
    ...
    env = {
        ...
        **real_env_values,
        "FI_DB_PATH": db_path,
        "FI_FORCE_ANOMALY": "true",
        "FI_DISCOVERY_SECURITY": "SYN1",
    }
